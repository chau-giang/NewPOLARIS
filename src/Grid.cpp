...
void CGridBasic::updateDataRange(cell_basic * cell, parameters & param)
{
    ...
    double a_alg = 0;
    double amaxJB_Lar = 0;
    double akrat_lowJ_min = 0;
    double akrat_lowJ_max = 0;
    double akrat_highJ_min = 0;
    double akrat_highJ_max = 0;
    double adisr = 0;
    double max_adisr = 0;
    ...
    double aine_lowJ = 0;
    double aine_highJ_lower = 0;
    double aine_highJ_upper = 0;
    double adg_lower = 0;
    double adg_upper = 0;
    ...
    if(param.getNrOfMixtures() > 1)
    {
    	dust_choices = param.getDustComponentChoices();
        for(uint i_id = 0; i_id < param.getNrOfMixtures(); i_id++)
        {
        	dust_id = dust_choices[i_id];
            if(dust_id > float(dust_id_max))
                dust_id_max = (double)dust_id;

            if(dust_id < float(dust_id_min))
                dust_id_min = (double)dust_id;
        }
    }
    ...
    {
        for(uint i_dens = 0; i_dens < data_pos_aine_lowJ_list.size(); i_dens++)
        {
    ...
    {
        for(uint i_dens = 0; i_dens < data_pos_aine_highJ_lower_list.size(); i_dens++)
        {
            aine_highJ_lower = cell->getData(data_pos_aine_highJ_lower_list[i_dens]);

            if(aine_highJ_lower > float(aine_highJ_lower_max))
                aine_highJ_lower_max = (double)aine_highJ_lower;

            if(aine_highJ_lower < float(aine_highJ_lower_min))
                aine_highJ_lower_min = (double)aine_highJ_lower;
        }
    }

    if(!data_pos_aine_highJ_upper_list.empty())
    {
        for(uint i_dens = 0; i_dens < data_pos_aine_highJ_upper_list.size(); i_dens++)
        {
            aine_highJ_upper = cell->getData(data_pos_aine_highJ_upper_list[i_dens]);

            if(aine_highJ_upper > float(aine_highJ_upper_max))
                aine_highJ_upper_max = (double)aine_highJ_upper;

            if(aine_highJ_upper < float(aine_highJ_upper_min))
                aine_highJ_upper_min = (double)aine_highJ_upper;
        }
    }
    ...
    {
        for(uint i_dens = 0; i_dens < data_pos_abs_ini_list.size(); i_dens++)
        {
    ...
    {
        for(uint i_dens = 0; i_dens < data_pos_akrat_lowJ_min_list.size(); i_dens++)
        {
            akrat_lowJ_min = cell->getData(data_pos_akrat_lowJ_min_list[i_dens]);

            if(akrat_lowJ_min > float(max_akrat_lowJ_min))
                max_akrat_lowJ_min = (double)akrat_lowJ_min;

            if(akrat_lowJ_min < float(min_akrat_lowJ_min))
                min_akrat_lowJ_min = (double)akrat_lowJ_min;
        }
    }
    
    if(!data_pos_akrat_lowJ_max_list.empty())
    {
        for(uint i_dens = 0; i_dens < data_pos_akrat_lowJ_max_list.size(); i_dens++)
        {
            akrat_lowJ_max = cell->getData(data_pos_akrat_lowJ_max_list[i_dens]);

            if(akrat_lowJ_max > float(max_akrat_lowJ_max))
                max_akrat_lowJ_max = (double)akrat_lowJ_max;

            if(akrat_lowJ_max < float(min_akrat_lowJ_max))
                min_akrat_lowJ_max = (double)akrat_lowJ_max;
        }
    }

    if(!data_pos_akrat_highJ_min_list.empty())
    {
        for(uint i_dens = 0; i_dens < data_pos_akrat_highJ_min_list.size(); i_dens++)
        {
            akrat_highJ_min = cell->getData(data_pos_akrat_highJ_min_list[i_dens]);

            if(akrat_highJ_min > float(max_akrat_highJ_min))
                max_akrat_highJ_min = (double)akrat_highJ_min;

            if(akrat_highJ_min < float(min_akrat_highJ_min))
                min_akrat_highJ_min = (double)akrat_highJ_min;
        }
    }
    ...
    {
        for(uint i_dens = 0; i_dens < data_pos_akrat_highJ_max_list.size(); i_dens++)
        {
            akrat_highJ_max = cell->getData(data_pos_akrat_highJ_max_list[i_dens]);

            if(akrat_highJ_max > float(max_akrat_highJ_max))
                max_akrat_highJ_max = (double)akrat_highJ_max;

            if(akrat_highJ_max < float(min_akrat_highJ_max))
                min_akrat_highJ_max = (double)akrat_highJ_max;
        }
    }
...
uint CGridBasic::validateDataPositions(parameters & param)
{
    ...
    if(nr_dust_id > 0 && (param.isMonteCarloSimulation() || param.getCommand() == CMD_DUST_EMISSION ||
                           ...
                           param.getCommand() == CMD_PROBING))
    {
        ...
        if(param.getNrOfMixtures() != 0)
        	nr_dust_id = param.getNrOfMixtures();
        ...
        {
            multi_temperature_entries += nr_dust_temp_sizes[i_id] + 1;
            stochastic_temperature_entries += nr_stochastic_sizes[i_id] + 1;
            wavelength_entries += nr_wavelength[i_id] + 1;
        }
...
void CGridBasic::printPhysicalParameters()
{
    ...
    if(!data_pos_aine_lowJ_list.empty())
        cout << "- aine_lowJ       (min,max) : [" << aine_lowJ_min << ", " << aine_lowJ_max << "] [m]" << endl;

    if(!data_pos_aine_highJ_lower_list.empty())
        cout << "- amin_ine_highJ  (min,max) : [" << aine_highJ_lower_min << ", " << aine_highJ_lower_max << "] [m]" << endl;
        
    if(!data_pos_aine_highJ_upper_list.empty())
        cout << "- amax_ine_highJ  (min,max) : [" << aine_highJ_upper_min << ", " << aine_highJ_upper_max << "] [m]" << endl;
    ...
    if(!data_pos_akrat_lowJ_min_list.empty())
        cout << "- akrat_lowJ   (min,max) : [" << min_akrat_lowJ_min << ", " << max_akrat_lowJ_min << "] [m]" << endl;
        
    if(!data_pos_akrat_highJ_min_list.empty())
        cout << "- akrat_highJ   (min,max) : [" << min_akrat_highJ_min << ", " << max_akrat_highJ_min << "] [m]" << endl;
	...
	//if(nr_mixture > 1)
    //if(data_pos_id != MAX_UINT) // if not definE GRIDdust_id in the binary file, data_pos_id = MAX_UINT (no define value), not print this line
    	                        // if we define GRIDdust_id in the binary file, data_pos_id != MAX_UINT (have value), print this line
...
bool CGridBasic::writeAMIRAFiles(string path, parameters & param, uint bins)
{
    ...
    string max_disr_filename = path + "adisr_max.am";
    ...
    string aine_lowJ_filename = path + "aine_lowJ.am";
    string aine_highJ_lower_filename = path + "aine_highJ_lower.am";
    string aine_highJ_upper_filename = path + "aine_highJ_upper.am";
    ...
    ofstream aine_lowJ_writer, aine_highJ_lower_writer, aine_highJ_upper_writer;
    ...
    if(plt_aine_lowJ)
    {
        aine_lowJ_writer.open(aine_lowJ_filename.c_str(), ios::out);

        if(aine_lowJ_writer.fail())
        {
            cout << "\nERROR: Cannot write to:\n " << aine_lowJ_filename << endl;
            return false;
        }
    }
    ...
    {
        aine_highJ_lower_writer.open(aine_highJ_lower_filename.c_str(), ios::out);

        if(aine_highJ_lower_writer.fail())
        {
            cout << "\nERROR: Cannot write to:\n " << aine_highJ_lower_filename << endl;
            return false;
        }
    }
    ...
    {
        aine_highJ_upper_writer.open(aine_highJ_upper_filename.c_str(), ios::out);

        if(aine_highJ_upper_writer.fail())
        {
            cout << "\nERROR: Cannot write to:\n " << aine_highJ_upper_filename << endl;
            return false;
        }
    }
        ...
        for(int y = -b_limit; y <= b_limit; y++)
        {
            ...
            for(int x = -b_limit; x <= b_limit; x++)
            {
                ...
                else
                {
                    ...
                    if(plt_aine_lowJ)
                        aine_lowJ_writer << float((getInelasticLowJRadius(pp, 0))) << endl;
                    
                    if(plt_aine_highJ_lower)
                        aine_highJ_lower_writer << float((getInelasticHighJLowerRadius(pp, 0))) << endl;
                        
				    if(plt_aine_highJ_upper)
                        aine_highJ_upper_writer << float((getInelasticHighJUpperRadius(pp, 0))) << endl;
                    ...
                    if(plt_akrat_lowJ_max)
                        akrat_lowJ_writer << float((getMaxkRATlowJRadius(pp, 0))) << endl;

                    if(plt_akrat_highJ_max)
                        akrat_highJ_writer << float((getMaxkRAThighJRadius(pp, 0))) << endl;
...
{
	// Note: In the case of two dust component, only write grain alignment size of the dust component dust_ID
	// So we need to define the first component to be silicate
    ...
    z_writer << "d\ta_alg\tamaxJB_Lar\tadisr\tmax_adisr\ta_barnet_low_J_lower\ta_barnet_low_J_upper\ta_barnet_high_J_lower\ta_barnet_high_J_upper\ta_dg_lower\ta_dg_upper\ta_dg_10_lower\ta_dg_10_upper\taine_lowJ\taine_highJ_lower\taine_highJ_upper"<< endl;
    ...
    while(next(pp))
    {
	    ...
	    if(!data_pos_aine_lowJ_list.empty())
            aine_lowJ = getInelasticLowJRadius(pp, 0);
            
        if(!data_pos_aine_highJ_lower_list.empty())
            aine_highJ_lower = getInelasticHighJLowerRadius(pp, 0);
            
        if(!data_pos_aine_highJ_upper_list.empty())
            aine_highJ_upper = getInelasticHighJUpperRadius(pp, 0);
        ...
        aine_lowJ << "\t" << aine_highJ_lower << "\t" << aine_highJ_upper << endl;
    ...
    y_writer << "d\ta_alg\tamaxJB_Lar\tadisr\tmax_adisr\ta_barnet_low_J_lower\ta_barnet_low_J_upper\ta_barnet_high_J_lower\ta_barnet_high_J_upper\ta_dg_lower\ta_dg_upper\ta_dg_10_lower\ta_dg_10_upper\taine_lowJ\taine_highJ_lower\taine_highJ_upper"<< endl;
    ...
    x_writer << "d\ta_alg\tamaxJB_Lar\tadisr\tmax_adisr\ta_barnet_low_J_lower\ta_barnet_low_J_upper\ta_barnet_high_J_lower\ta_barnet_high_J_upper\ta_dg_lower\ta_dg_upper\ta_dg_10_lower\ta_dg_10_upper\taine_lowJ\taine_highJ_lower\taine_highJ_upper"<< endl;
    while(next(pp))
    {
        double pos = pp->getPosition().X();
        double a_alg = 0;
...
bool CGridBasic::writeMidplaneFits(string data_path, parameters & param, uint bins, bool all, uint loop)
{
    bool res = true;
    ...
    if(all)
    {
        ...
        //plt_dust_id = (!data_pos_id_list.empty());
        ...
        if(cmd != CMD_RAT && cmd != CMD_TEMP_RAT && cmd != CMD_TEMP_RAT_DISR && cmd != CMD_RAT_DISR)
        {
            ...
            {
                plt_aine_lowJ = (!data_pos_aine_lowJ_list.empty()) && param.isInPlotList(GRIDaine_lowJ);
                plt_aine_highJ_lower = (!data_pos_aine_highJ_lower_list.empty()) && param.isInPlotList(GRIDaine_highJ_lower); 
                plt_aine_highJ_upper = (!data_pos_aine_highJ_upper_list.empty()) && param.isInPlotList(GRIDaine_highJ_upper);
            }  
            if (param.getAligkRAT())
            {
            	plt_akrat_lowJ_min = (!data_pos_akrat_lowJ_min_list.empty()) && param.isInPlotList(GRIDakrat_lowJ_min);
            	plt_akrat_lowJ_max = (!data_pos_akrat_lowJ_max_list.empty()) && param.isInPlotList(GRIDakrat_lowJ_max);
            	plt_akrat_highJ_min = (!data_pos_akrat_highJ_min_list.empty()) && param.isInPlotList(GRIDakrat_highJ_min);
            	plt_akrat_highJ_max = (!data_pos_akrat_highJ_max_list.empty()) && param.isInPlotList(GRIDakrat_highJ_max);
            }
        }
    ...
    else
    {
        ...
        if(cmd == CMD_RAT || cmd == CMD_TEMP_RAT || cmd == CMD_RAT_DISR || cmd == CMD_TEMP_RAT_DISR)
        {
            ...
            if (param.getAligInelastic())
            {
	 	    ...
	 	    if (param.getAligkRAT())
	 	    {
	        	plt_akrat_lowJ_min  = param.isInPlotList(GRIDakrat_lowJ_min);
	        	plt_akrat_lowJ_max  = param.isInPlotList(GRIDakrat_lowJ_max);
	        	plt_akrat_highJ_min  = param.isInPlotList(GRIDakrat_highJ_min);
	        	plt_akrat_highJ_max  = param.isInPlotList(GRIDakrat_highJ_max);
		    }
        }
    ...
    uint nr_parameters = uint(plt_gas_dens) + uint(plt_dust_dens) + uint(plt_gas_temp) + uint(plt_dust_temp) +
                         ...
                         uint(plt_barnet_high_lower) + uint(plt_barnet_high_upper) + uint(plt_aine_lowJ) + uint(plt_aine_highJ_lower) +
                         uint(plt_aine_highJ_upper) + uint(plt_dg_lower) + uint(plt_dg_upper) + uint(plt_dg_10_lower) + 
                         uint(plt_dg_10_upper) +  uint(plt_amaxJB_Lar) + uint(plt_abs_ini) + uint(plt_akrat_lowJ_min) + 
                         uint(plt_akrat_lowJ_max) + uint(plt_akrat_highJ_min) + uint(plt_akrat_highJ_max);
    ...
    //        nr_parameters += nr_densities;
    //if(plt_dust_id)
    //    if(nr_dust_id > 1 && data_pos_id_list.size() >= nr_dust_id)
    //        nr_parameters += nr_dust_id;
    if(plt_dust_temp)
    ...
    valarray<double> array_dust_id(nelements);
    ...
    valarray<double> array_aine_lowJ(nelements);
    valarray<double> array_aine_highJ_lower(nelements);
    valarray<double> array_aine_highJ_upper(nelements);
    ...
    valarray<double> array_akrat_lowJ_min(nelements);
    valarray<double> array_akrat_lowJ_max(nelements);
    valarray<double> array_akrat_highJ_min(nelements);
    valarray<double> array_akrat_highJ_max(nelements);
    ...
    if(plt_gas_dens)
    {
        ...
        for(int i_cell = 0; i_cell < nelements; i_cell++)
        {
            ...
            if(nr_densities > 1 && data_pos_gd_list.size() == nr_densities)
            	...
            	cout << "ERROR: Only one gas density distribution should be defined in the grid" << endl;
            else
            	buffer_gas_dens[i_cell] = new double[nr_densities];
                // buffer_gas_dens == matrix[N_cell][nr_density], nr_densities = 1 
        }
    }
    ...
    if(plt_dust_dens)
    {
        ...
        for(int i_cell = 0; i_cell < nelements; i_cell++)
        {
            ...
            // in the grid
            if(nr_densities > 1 && data_pos_dd_list.size() == nr_densities)
            	cout << "ERROR: Only one dust density distribution should be defined in the grid" << endl;
    ...
    if(plt_gas_temp)
        buffer_gas_temp = new double[nelements]; //buffer_Gas_temp = array[N_cell]
    ...
    if(plt_dust_temp)
    {
        ...
        for(int i_cell = 0; i_cell < nelements; i_cell++)
        {
            ...
            // in the grid
            if(nr_dust_id > 1 && data_pos_dt_list.size() >= nr_dust_id)
                buffer_dust_temp[i_cell] = new double[nr_dust_id + 1]; // buffer_dust_temp = [<Td>, Td(ID=1), Td(ID=2), ..., Td(ID=n)]
            else
            	buffer_dust_temp[i_cell] = new double[nr_dust_id]; //buffer_dust_temp = [<Td>]
        }
    }
    ...
    if(plt_rat)
    {
        ...
        for(int i_cell = 0; i_cell < nelements; i_cell++)
            ...
            // buffer_rat == Matrix[N_cell][Nr_dust_id]
    }
    ...
    //if(plt_dust_id)
    //{
    //    buffer_dust_id = new double *[nelements];
    //    for(int i_cell = 0; i_cell < nelements; i_cell++)
    //        buffer_dust_id[i_cell] = new double[data_pos_id_list.size()];
    //}
    ...
    if(plt_rad_field1)
    {
        ...
        for(int i_cell = 0; i_cell < nelements; i_cell++)
        {
            ...
            for(int wID = 0; wID < WL_STEPS; wID++)
                buffer_rad_field[i_cell][wID] = new double[nr_rad_field_comp]; // buffer_rad_field = Matrix[Nr_cell][Nr_wave][ux - uy - uz - urad]
        }
    }
    ...
    if(plt_aine_lowJ)
    {
        buffer_aine_lowJ = new double *[nelements];
        for(int i_cell = 0; i_cell < nelements; i_cell++)
            buffer_aine_lowJ[i_cell] = new double[data_pos_aine_lowJ_list.size()];
    }
    
    if(plt_aine_highJ_lower)
    {
        buffer_aine_highJ_lower = new double *[nelements];
        for(int i_cell = 0; i_cell < nelements; i_cell++)
            buffer_aine_highJ_lower[i_cell] = new double[data_pos_aine_highJ_lower_list.size()];
    }
    
    if(plt_aine_highJ_upper)
    {
        buffer_aine_highJ_upper = new double *[nelements];
        for(int i_cell = 0; i_cell < nelements; i_cell++)
            buffer_aine_highJ_upper[i_cell] = new double[data_pos_aine_highJ_upper_list.size()];
    }
    ...
    {
        buffer_akrat_lowJ_min = new double *[nelements];
        for(int i_cell = 0; i_cell < nelements; i_cell++)
            buffer_akrat_lowJ_min[i_cell] = new double[data_pos_akrat_lowJ_min_list.size()];
    }

    if(plt_akrat_lowJ_max)
    {
        buffer_akrat_lowJ_max = new double *[nelements];
        for(int i_cell = 0; i_cell < nelements; i_cell++)
            buffer_akrat_lowJ_max[i_cell] = new double[data_pos_akrat_lowJ_max_list.size()];
    }
    
    if(plt_akrat_highJ_min)
    {
        buffer_akrat_highJ_min = new double *[nelements];
        for(int i_cell = 0; i_cell < nelements; i_cell++)
            buffer_akrat_highJ_min[i_cell] = new double[data_pos_akrat_highJ_min_list.size()];
    }
    
    if(plt_akrat_highJ_max)
    {
        buffer_akrat_highJ_max = new double *[nelements];
        for(int i_cell = 0; i_cell < nelements; i_cell++)
            buffer_akrat_highJ_max[i_cell] = new double[data_pos_akrat_highJ_max_list.size()];
    }
    ...
    if(plt_abs_ini)
    {
        ...
        for(int i_cell = 0; i_cell < nelements; i_cell++)
        {
            ...
            // in the grid
            if(nr_dust_id > 1 && data_pos_abs_ini_list.size() >= nr_dust_id)
                buffer_abs_ini[i_cell] = new double[nr_dust_id + 1];
            else
                buffer_abs_ini[i_cell] = new double[nr_dust_id];
        }
    }
...
#pragma omp parallel for schedule(dynamic)
            for(long i_cell = 0; i_cell < nelements; i_cell++)
            {
                ...
                fillMidplaneBuffer(tx, ty, tz, i_cell, param, loop);
...
#pragma omp critical
            ...
            {
                if(nr_densities > 1 && data_pos_gd_list.size() >= nr_densities)
                	cout << "ERROR: Grid should contains only one gas density distribution" << endl;
                else
                {
		        	for(int i_cell = 0; i_cell < nelements; i_cell++)
                ...
                //    {
                //        for(int i_cell = 0; i_cell < nelements; i_cell++)
                //            array_gas_dens[i_cell] = buffer_gas_dens[i_cell][i_density];
                //        fpixel[3]++;
                //        pFits->pHDU().write(fpixel, nelements, array_gas_dens);
                //    }
                //}
            ...
            {
                for(int i_cell = 0; i_cell < nelements; i_cell++)
                    array_dust_temp[i_cell] = buffer_dust_temp[i_cell][0]; // first element is for the average dust temperature
                fpixel[3]++;
                ...
                if(nr_dust_id > 1 && data_pos_dt_list.size() >= nr_dust_id)
                    ...
                    {
                        for(int i_cell = 0; i_cell < nelements; i_cell++)
                            array_dust_temp[i_cell] = buffer_dust_temp[i_cell][i_id + 1]; // another is for dust temperature of dust mixture ID_dust
                        fpixel[3]++;
                        pFits->pHDU().write(fpixel, nelements, array_dust_temp);
                    }
            }
            ...
            if(plt_rat)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_rat[i_cell] = buffer_rat[i_cell][i_id];
            ...
            if(plt_dust_id)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_dust_id[i_cell] = buffer_dust_id[i_cell][i_id];

                    fpixel[3]++;
                    pFits->pHDU().write(fpixel, nelements, array_dust_id);
                }
            ...
            if(plt_disr)
            {
                for(uint i_id = 0; i_id < nr_dust_id; i_id++)
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                    {
                        array_disr[i_cell] = buffer_disr[i_cell][i_id];
                        //cout << array_disr[i_cell] << endl;
		    }
            ...
            if(plt_max_disr)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_max_disr[i_cell] = buffer_max_disr[i_cell][i_id];
            ...
            if(plt_param_modif)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_param_modif[i_cell] = buffer_param_modif[i_cell][i_id];
            ...
            if(plt_barnet_low_lower)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_barnet_low_lower[i_cell] = buffer_barnet_low_lower[i_cell][i_id];
            ...
            if(plt_barnet_low_upper)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_barnet_low_upper[i_cell] = buffer_barnet_low_upper[i_cell][i_id];
            ...
            if(plt_barnet_high_lower)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_barnet_high_lower[i_cell] = buffer_barnet_high_lower[i_cell][i_id];
            ...
            if(plt_barnet_high_upper)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_barnet_high_upper[i_cell] = buffer_barnet_high_upper[i_cell][i_id];
            ...
            if(plt_aine_lowJ)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_aine_lowJ[i_cell] = buffer_aine_lowJ[i_cell][i_id];

                    fpixel[3]++;
                    pFits->pHDU().write(fpixel, nelements, array_aine_lowJ);
                }
            }
            ...
            if(plt_aine_highJ_lower)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_aine_highJ_lower[i_cell] = buffer_aine_highJ_lower[i_cell][i_id];

                    fpixel[3]++;
                    pFits->pHDU().write(fpixel, nelements, array_aine_highJ_lower);
                }
            }
            ...
            if(plt_aine_highJ_upper)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_aine_highJ_upper[i_cell] = buffer_aine_highJ_upper[i_cell][i_id];

                    fpixel[3]++;
                    pFits->pHDU().write(fpixel, nelements, array_aine_highJ_upper);
                }
            }
            ...
            if(plt_dg_lower)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_dg_lower[i_cell] = buffer_dg_lower[i_cell][i_id];
            ...
            if(plt_dg_upper)
            {
               for(uint i_id = 0; i_id < nr_dust_id; i_id++)
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_dg_upper[i_cell] = buffer_dg_upper[i_cell][i_id];
            ...
            if(plt_dg_10_lower)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_dg_10_lower[i_cell] = buffer_dg_10_lower[i_cell][i_id];
            ...
            if(plt_dg_10_upper)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_dg_10_upper[i_cell] = buffer_dg_10_upper[i_cell][i_id];
            ...
            if(plt_amaxJB_Lar)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_amaxJB_Lar[i_cell] = buffer_amaxJB_Lar[i_cell][i_id];
            ...
            if(plt_akrat_lowJ_min)
            {
               for(uint i_id = 0; i_id < nr_dust_id; i_id++)
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_akrat_lowJ_min[i_cell] = buffer_akrat_lowJ_min[i_cell][i_id];

                    fpixel[3]++;
                    pFits->pHDU().write(fpixel, nelements, array_akrat_lowJ_min);
                }
            }
            ...
            if(plt_akrat_lowJ_max)
            {
               for(uint i_id = 0; i_id < nr_dust_id; i_id++)
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_akrat_lowJ_max[i_cell] = buffer_akrat_lowJ_max[i_cell][i_id];

                    fpixel[3]++;
                    pFits->pHDU().write(fpixel, nelements, array_akrat_lowJ_max);
                }
            }
            ...
            if(plt_akrat_highJ_min)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_akrat_highJ_min[i_cell] = buffer_akrat_highJ_min[i_cell][i_id];

                    fpixel[3]++;
                    pFits->pHDU().write(fpixel, nelements, array_akrat_highJ_min);
                }
            }
            ...
            if(plt_akrat_highJ_max)
            {
                ...
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        array_akrat_highJ_max[i_cell] = buffer_akrat_highJ_max[i_cell][i_id];

                    fpixel[3]++;
                    pFits->pHDU().write(fpixel, nelements, array_akrat_highJ_max);
                }
            }
            ...
            if(plt_abs_ini)
            {
                ...
                fpixel[3]++;
                pFits->pHDU().write(fpixel, nelements, array_abs_ini); // Average initial absoprtion rate
                ...
                if(nr_dust_id > 1 && data_pos_abs_ini_list.size() >= nr_dust_id)
                {
				    ...
				    {
				        for(int i_cell = 0; i_cell < nelements; i_cell++)
				            array_abs_ini[i_cell] = buffer_abs_ini[i_cell][i_id+1]; // Initial absorption rate of dust mixture ID_i
...
#pragma omp critical
            ...
            if(plt_gas_dens)
            {
                ...
                if(nr_densities > 1 && data_pos_gd_list.size() >= nr_densities)
                	cout << "ERROR: Only one gas density distribution should be in the grid" << endl;
                else 
                    for(uint i_density = 0; i_density < nr_densities; i_density++)
                    {
                        for(int i_cell = 0; i_cell < nelements; i_cell++)
            ...
            if(plt_dust_temp)
            {
                ...
                if(nr_dust_id > 1 && data_pos_dt_list.size() >= nr_dust_id)
                    ...
                    {
                        for(int i_cell = 0; i_cell < nelements; i_cell++)
                            array_dust_temp[i_cell] = buffer_dust_temp[i_cell][i_id + 1]; 
                        fpixel[3]++;
                        pFits->pHDU().write(fpixel, nelements, array_dust_temp);
                    }
            }
            ...
            if(plt_abs_ini)
            {
                ...
                for(uint i_id = 0; i_id < nr_dust_id; i_id++)
                {
                    for(int i_cell = 0; i_cell < nelements; i_cell++)
                        ...
                        array_abs_ini[i_cell] = buffer_abs_ini[i_cell][i_id];
                    fpixel[3]++;
                    pFits->pHDU().write(fpixel, nelements, array_abs_ini);
                }
                //}
    ...
    if(plt_dust_temp)
    {
        ...
        if(nr_dust_id > 1)
        {
            ...
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("dust_temperature_%i [K]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        }
        else
    ...
    if(plt_rat)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("rat_aalig_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
    ...
    if(plt_disr)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("adisr_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
    ...
    if(plt_max_disr)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("adisr_max_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
            counter++;
            updateMidplaneString(str_1, str_2, counter);
            pFits->pHDU().addKey(str_1, "adisr_max [m]", str_2);
        }
    }
    ...
    if(plt_param_modif)
    {
        if(nr_dust_id > 1)
        {
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("param_modify_%i", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        }
        else
    ...
    if(plt_barnet_low_lower)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("amin_aJ_lowJ_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
    ...
    if(plt_barnet_low_upper)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("amax_aJ_lowJ_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
    ...
    if(plt_barnet_high_lower)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("amin_aJ_highJ_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
    ...
    if(plt_barnet_high_upper)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("amax_aJ_highJ_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
    ...
    if(plt_aine_lowJ)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("aine_lowJ_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
            counter++;
            updateMidplaneString(str_1, str_2, counter);
            pFits->pHDU().addKey(str_1, "aine_lowJ [m]", str_2);
        }
    }
    ...
    if(plt_aine_highJ_lower)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("aine_highJ_lower%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
            counter++;
            updateMidplaneString(str_1, str_2, counter);
            pFits->pHDU().addKey(str_1, "aine_highJ_lower [m]", str_2);
        }
    }
    ...
    if(plt_aine_highJ_upper)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("aine_highJ_upper_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
            counter++;
            updateMidplaneString(str_1, str_2, counter);
            pFits->pHDU().addKey(str_1, "aine_highJ_upper [m]", str_2);
        }
    }
    ...
    if(plt_dg_lower)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("aminJB_DG_0.5%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
    ...
    if(plt_dg_upper)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("amaxJB_DG_0.5_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
    ...
    if(plt_dg_10_lower)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("aminJB_DG_1_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
    ...
    if(plt_dg_10_upper)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("amaxJB_DG_1_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
    ...
    if(plt_amaxJB_Lar)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("amaxJB_Lar_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
    ...
    if(plt_akrat_lowJ_min)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("akrat_lowJ_min_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
            counter++;
            updateMidplaneString(str_1, str_2, counter);
            pFits->pHDU().addKey(str_1, "akrat_lowJ_min [m]", str_2);
        }
    }
    ...
    if(plt_akrat_lowJ_max)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("akrat_lowJ_max_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
            counter++;
            updateMidplaneString(str_1, str_2, counter);
            pFits->pHDU().addKey(str_1, "akrat_lowJ_max [m]", str_2);
        }
    }
    ...
    if(plt_akrat_highJ_min)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("akrat_highJ_min_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
            counter++;
            updateMidplaneString(str_1, str_2, counter);
            pFits->pHDU().addKey(str_1, "akrat_highJ_min [m]", str_2);
        }
    }
    ...
    if(plt_akrat_highJ_max)
    {
        if(nr_dust_id > 1)
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("akrat_highJ_max_%i [m]", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        else
        {
            counter++;
            updateMidplaneString(str_1, str_2, counter);
            pFits->pHDU().addKey(str_1, "akrat_highJ_max [m]", str_2);
        }
    }
    ...
    if(plt_abs_ini)
    {
        ...
        if(nr_dust_id > 1)
        {
            for(uint i_id = 1; i_id <= nr_dust_id; i_id++)
            {
                counter++;
                updateMidplaneString(str_1, str_2, counter);
                string str_3 = getDensityString("initial_absorption_rate_%i", i_id);
                pFits->pHDU().addKey(str_1, str_3, str_2);
            }
        }
        else
    ...
    if(plt_aine_lowJ)
        delete[] buffer_aine_lowJ;
    if(plt_aine_highJ_lower)
        delete[] buffer_aine_highJ_lower;
    if(plt_aine_highJ_upper)
        delete[] buffer_aine_highJ_upper;
    if(plt_dg_lower)
    ...
    if(plt_akrat_lowJ_min)
        delete[] buffer_akrat_lowJ_min;
    if(plt_akrat_lowJ_max)
        delete[] buffer_akrat_lowJ_max;
    if(plt_akrat_highJ_min)
        delete[] buffer_akrat_highJ_min;
    if(plt_akrat_highJ_max)
        delete[] buffer_akrat_highJ_max;
