...
class CMathFunctions
{
  public:
    ...
    static inline double calc_amaxJB_Lar_default(double B, double Td, double Tg, double ng, double s, double larm_f)
    {
        ...
        return s * s * B / den / larm_f / reduced_factor;
    }
    ...
    static inline double tau_larmor_para(double fp, double a, double rho, double s, double Td, double B)
    {
        //*******************************************************************
        //*      Args: fp: iron fraction of PM grains
        //*            a: major axis of oblate grain (cm)
        //*		rho: grain mass density ( g cm-3)
        //*            Td: dust temperature (K)
        //*            s: aspect ratio = minor axis/major axis
        //*            B: magnetic field strength (G)
        //*
        //*     Returns:
        //*  tau_lar = 2*PI*I*ge / X(0)*V*B: Larmor precession timescale of PM grains
        //********************************************************************
        double I_para = 8. * PI / 15. * rho * s * pow(a, 5);
        double V = 4./3 * PI * s * pow(a, 3);
        double ge = 2;
        double uB = 9.27401e-21;    // Bohr magneton [in cgs]
        double hbar = 1.05457182e-27; // reduced Planck constant [in cgs]
        double gamma_e = ge*uB / hbar;

        double chi_0 = calc_X_0(Td, fp);
        
        return 2 * PI * I_para * gamma_e / (chi_0 * V * B);
    }

    static inline double calc_amaxJB_Lar_para(double B, double Td, double Tg, double ng, double s, double fp)
    {
        //*******************************************************************
        ...
        //     Calculation in the SI unit because I take it from analytical fomula
        //**************************************************************************
 	...
 	static inline double tau_larmor_super(double Ncl, double phi_sp, double a, double rho, double s, double Td, double B)
    {
        //*******************************************************************
        //*      Args: Ncl: number of iron per cluster
        ...
        double ge = 2;
        double uB = 9.27401e-21;    // Bohr magneton
        double hbar = 1.05457182e-27; // reduced Planck constant
        double gamma_e = ge*uB / hbar;

        double chi_0_spm = calc_X_0_super(Td, Ncl, phi_sp, s);
        
        return 2 * PI * I_para * gamma_e / (chi_0_spm * V * B);
    }
    
    static inline double calc_amaxJB_Lar_super(double B, double Td, double Tg, double ng, double s, double Ncl, double phi_sp)
    {
    	//*******************************************************************
    	//*		 Input: B: magnetic field strength (T)
    	//*	   		    Td: dust temperature (K)
		...
		// 	       Calculation in the SI unit, taken from the analytical fomula
		//**************************************************************************
