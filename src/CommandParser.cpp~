...
bool CCommandParser::parseLine(parameters * param, string cmd, string data, uint id)
{
    ...
    if(cmd.compare("<spheroidal_size>") == 0)
    {
        param->setSpheroidalSize(atob(atoi(data.c_str())));
        return true;
    }
    ...
    if(cmd.compare("<dust_component>") == 0 || cmd.compare("<dust_component id = >") == 0)
    {
        ...
        uint dust_component_choice = 0; // If dont define the dust component id, id = 0 (default value)
    ...
    if(cmd.compare("<Qe3>") == 0)
    {
        param->setQe3(atof(data.c_str()));
        return true;
    }
