...
class CDustComponent
{
  public:
    CDustComponent()
    {
        ...
        min_akrat_lowJ = 1e200;
        max_akrat_lowJ = 0;
        min_akrat_highJ = 1e200;
        max_akrat_highJ = 0;
    ...
    // ------------------------------------------------------------------------------------------------------------------------
    double getCext1(CGridBasic * grid, cell_basic * cell, uint i_density, uint wID)
    {
        // Get local min and max grain sizes
    ...
    double getCext2(CGridBasic * grid, cell_basic * cell, uint i_density, uint wID)
    {
        // Get local min and max grain sizes
        double a_min = getSizeMin(grid, cell);
        double a_max = getSizeMax(grid, cell);
    ...
    double getCabs1(CGridBasic * grid, cell_basic * cell, uint i_density, uint wID)
    {
        // Get local min and max grain sizes
        double a_min = getSizeMin(grid, cell);
        double a_max = getSizeMax(grid, cell);
    ...
    double getCabs2(CGridBasic * grid, cell_basic * cell, uint i_density, uint wID)
    {
        // Get local min and max grain sizes
        double a_min = getSizeMin(grid, cell);
        double a_max = getSizeMax(grid, cell);
    ...
    double getCsca1(CGridBasic * grid, cell_basic * cell, uint i_density, uint wID)
    {
        // Get local min and max grain sizes
        double a_min = getSizeMin(grid, cell);
        double a_max = getSizeMax(grid, cell);
    ...
    double getCsca2(CGridBasic * grid, cell_basic * cell, uint i_density, uint wID)
    {
        // Get local min and max grain sizes
        double a_min = getSizeMin(grid, cell);
        double a_max = getSizeMax(grid, cell);
    ...
    double getCextMean(CGridBasic * grid, cell_basic * cell, uint i_density, uint wID)
    {
        return (2.0 * getCext1(grid, cell, i_density, wID) + getCext2(grid, cell, i_density, wID)) / 3.0;
    }

    double getCabsMean(CGridBasic * grid, cell_basic * cell, uint i_density, uint wID)
    {
        return (2.0 * getCabs1(grid, cell, i_density, wID) + getCabs2(grid, cell, i_density, wID)) / 3.0;
    }

    double getCscaMean(CGridBasic * grid, cell_basic * cell, uint i_density, uint wID)
    {
        return (2.0 * getCsca1(grid, cell, i_density, wID) + getCsca2(grid, cell, i_density, wID)) / 3.0;
    }
    ...
    // -----------------------------------------------------------------------------
    double getNewMeanCext(CGridBasic * grid, photon_package * pp, uint i_density)
    {
        return grid->getNewMeanCext(pp, i_density);
    }

    double getNewMeanCabs(CGridBasic * grid, photon_package * pp, uint i_density)
    {
        return grid->getNewMeanCabs(pp, i_density);
    }

    double getNewMeanCsca(CGridBasic * grid, photon_package * pp, uint i_density)
    {
        return grid->getNewMeanCsca(pp, i_density);
    }
...
//*************************************
    double getMinkRATlowJRadius()
    {
        return min_akrat_lowJ;
    }

    double getMaxkRATlowJRadius()
    {
        return max_akrat_lowJ;
    }


//*************************************
    double getMinkRAThighJRadius()
    {
        return min_akrat_highJ;
    }

    double getMaxkRAThighJRadius()
    {
        return max_akrat_highJ;
    }
...
//*************************************   
    ...
    void scatter(CGridBasic * grid, photon_package * pp, uint i_density, bool adjust_stokes = false, uint loop = 1)
    {
        switch(phID)
        {
            case PH_HG:
            ...
            default:
                ...
                if(adjust_stokes > 0)
                {
                    ...
                    double albedo;
                    if (loop == 1)
                    	albedo = getCscaMean(grid, pp, i_density) / getCextMean(grid, pp, i_density);
                    else
                    	albedo = getNewMeanCext(grid, pp, i_density) * 0.5 / getNewMeanCext(grid, pp, i_density);
                    S *= albedo / PIx4;
...
//*******************************************************************************
    ...
    void calcNewMeanEfficiency(CGridBasic * grid, cell_basic * cell, uint i_density);
  ...
  private:
    ...
    double min_akrat_lowJ, max_akrat_lowJ;
    double min_akrat_highJ, max_akrat_highJ;
...
class CDustMixture
{
  public:
    ...
    double getNewMeanCextGrid(CGridBasic * grid, photon_package * pp)
    {
        double sum = 0;
        if(mixed_component != 0)
        {
    ...
    double getNewMeanCabsGrid(CGridBasic * grid, photon_package * pp)
    {
        double sum = 0;
        if(mixed_component != 0)
        {
    ...
    double getNewMeanCscaGrid(CGridBasic * grid, photon_package * pp)
    {
        double sum = 0;
        if(mixed_component != 0)
        {
    ...
    void calcMaxAlignedRadii(CGridBasic * grid, cell_basic * cell)
    {
        if(mixed_component != 0)
        {
            if(grid->useDustChoice())
            {
                uint i_mixture = getMixtureID(grid, cell);
                mixed_component[i_mixture].calcMaxAlignedRadii(grid, cell, 0);
            }
            else
                for(uint i_mixture = 0; i_mixture < getNrOfMixtures(); i_mixture++)
                    mixed_component[i_mixture].calcMaxAlignedRadii(grid, cell, i_mixture);
        }
    }
    ...
    void calcNewMeanEfficiency(CGridBasic * grid, cell_basic * cell)
    {
        if(mixed_component != 0)
        {
            if(grid->useDustChoice())
            {
                uint i_mixture = getMixtureID(grid, cell);
                mixed_component[i_mixture].calcNewMeanEfficiency(grid, cell, 0);
            }
            else
                for(uint i_mixture = 0; i_mixture < getNrOfMixtures(); i_mixture++)
                    mixed_component[i_mixture].calcNewMeanEfficiency(grid, cell, i_mixture);
        }
    }
...
//******************************************************************************
    double getMinkRATlowJRadius()
    {
        double min_akrat_lowJ = 1e200;
        if(mixed_component != 0)
            for(uint i_mixture = 0; i_mixture < getNrOfMixtures(); i_mixture++)
            {
                double akrat_lowJ = mixed_component[i_mixture].getMinkRATlowJRadius();
                if(akrat_lowJ < min_akrat_lowJ)
                    min_akrat_lowJ = akrat_lowJ;
            }
        return min_akrat_lowJ;
    }
    ...
    double getMaxkRATlowJRadius()
    {
        double max_akrat_lowJ = 0;
        if(mixed_component != 0)
            for(uint i_mixture = 0; i_mixture < getNrOfMixtures(); i_mixture++)
            {
                double akrat_lowJ = mixed_component[i_mixture].getMaxkRATlowJRadius();
                if(akrat_lowJ > max_akrat_lowJ)
...
//******************************************************************************
    double getMinkRAThighJRadius()
    {
        double min_akrat_highJ = 1e200;
        if(mixed_component != 0)
            for(uint i_mixture = 0; i_mixture < getNrOfMixtures(); i_mixture++)
            {
                double akrat_highJ = mixed_component[i_mixture].getMinkRAThighJRadius();
                if(akrat_highJ < min_akrat_highJ)
                    min_akrat_highJ = akrat_highJ;
            }
        return min_akrat_highJ;
    }
    ...
    double getMaxkRAThighJRadius()
    {
        double max_akrat_highJ = 0;
        if(mixed_component != 0)
            for(uint i_mixture = 0; i_mixture < getNrOfMixtures(); i_mixture++)
            {
                double akrat_highJ = mixed_component[i_mixture].getMaxkRAThighJRadius();
                if(akrat_highJ > max_akrat_highJ)
                    max_akrat_highJ = akrat_highJ;
            }
        return max_akrat_highJ;
    }
...
//******************************************************************************
    ...
    uint getScatteringMixture(CGridBasic * grid, photon_package * pp, uint loop = 1)
    {
        if(mixed_component != 0)
        {
            if(grid->useDustChoice())
            ...
            else
            {
                ...
                for(uint i_mixture = 0; i_mixture < getNrOfMixtures(); i_mixture++)
                {
                    if (loop == 1)
                    	...
                    	pb -= getRelativeDustNumberDensity(grid, pp, i_mixture) *
                          	mixed_component[i_mixture].getNewMeanCsca(grid, pp, i_mixture) / getNewMeanCscaGrid(grid, pp);	
                    if(rnd > pb)
                        return i_mixture;
                }
            }
    ...
    void scatter(CGridBasic * grid, photon_package * pp, bool adjust_stokes = false, uint loop = 1)
    {
        if(mixed_component != 0)
        {
            uint i_mixture = getScatteringMixture(grid, pp, loop);
            mixed_component[i_mixture].scatter(grid, pp, adjust_stokes, loop);
        }
    }
