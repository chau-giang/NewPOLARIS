...
bool CPipeline::calcMonteCarloRadiationField(parameters & param)
{
    ...
    if((param.isMonteCarloSimulation() && param.getSaveRadiationField()) || param.getStochasticHeatingMaxSize() > 0) 
		//if(param.getSaveRadiationField() || param.isMonteCarloSimulation() || param.getStochasticHeatingMaxSize() > 0)
		   use_energy_density = true;
		   
		   
	// ======= Location for radiation field and <Cext>new, <Cabs>new, <Csca>new =============
	uint _data_len;
	// For the first time of calculating MCRT, use 400 places for storing radiation field
 
	if (use_energy_density) // if radaition field will be stored inside the grid
		_data_len = 4 * WL_STEPS;
	else
		if (param.isMonteCarloSimulation()) // if radiation field is not store, but still calc MCRT
			_data_len = WL_STEPS;
		else
			_data_len = 0; // only calculate rat and disr using results from MCRT simulation
	
	if (param.getMCRTloop() > 1) // For the 2+ times of calculating MCRT simulation
		_data_len += 3*WL_STEPS + 1;
		

	cout << "data len" << _data_len << endl;
    ...
    //if(param.isTemperatureSimulation() && param.getMCRTloop() == 1)
    //{
    //	cout << "Convert dust/gas temperature to ini_abs_rate" << endl;
    //    if(param.getDustOffset())
    //        rad.convertTempInQB(param.getOffsetMinGasDensity(), false, param);
    //    else if(param.getDustGasCoupling())
    //        rad.convertTempInQB(param.getOffsetMinGasDensity(), true, param);
    //}
    ...
    //{
	    ...
	    //	cout << "\n distribution by RATD. If you dont take into account this effect, put <MCRT_loop> 1. " << endl;
	    //	cout << "\n Otherwise, change the simulation mode to <cmd> CMD_TEMP_DISR or <cmd> CMD_TEMP_RAT_DISR" << endl;
	 	//return false;   		 
	    //}	
	...
	if (param.isMonteCarloSimulation())  
	{
		// calculate new cross section in case of second loop
		if(mcrt_loop != 1)
		{
			rad.calcNewMeanEfficiency(param);	
    	}
    	...
    	rad.calcMonteCarloRadiationField(param.getCommand(),
    			     			param,
			                     	use_energy_density,
			                     	false, 
			                     	mcrt_loop); //(param.getCommand() == CMD_RAT));
    }
    	
	// Calculation of dust temperature
	if(param.isTemperatureSimulation()) 
   	    rad.calcFinalTemperature(use_energy_density, param, mcrt_loop);
    ...
    if(param.isRatSimulation())
	{
		...
		if (param.getAligkRAT())
    	{
            ...
            rad.calckRAThighJRadii(param, mcrt_loop);
    	}
	}
	
	// Calculation of disruption
	if(param.isRATDSimulation())
    {       
            rad.calcDisruptRadii(param, mcrt_loop);
       	    rad.calcMaxDisruptRadii(param, mcrt_loop);
            rad.calcSizeParamModify(param);
	}
	
	// Update radiation field information inside the grid for the second loop
 	if ((param.isMonteCarloSimulation()) && (mcrt_loop > 1)) 	 
		grid->resetRadiationField();  
		
    cout << SEP_LINE;
 
    
    if(!grid->writeMidplaneFits(path_data + "output_", param, param.getOutMidDataPoints(), false, mcrt_loop))
        return false;
    ...
    if(param.getSaveRadiationField())
    	grid->saveRadiationField(mcrt_loop);
    if(param.isTemperatureSimulation())
...
bool CPipeline::assignDustMixture(parameters & param, CDustMixture * dust, CGridBasic * grid)
{
    ...
    if(!dust->writeComponentInfo(path_data, path_plot))
        return false;
