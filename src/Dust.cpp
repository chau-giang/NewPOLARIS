...
uint CDustComponent::getInteractingDust(CGridBasic * grid, photon_package * pp, uint i_density, uint cross_section)
{
    ...
    switch(cross_section)
    {
        case CROSS_ABS:
            if(abs_prob != 0)
                return findSizeID(pp, abs_prob[w], a_min, a_disr, a_disr_max, a_max);
            break;

        case CROSS_SCA:
            if(sca_prob != 0)
                return findSizeID(pp, sca_prob[w], a_min, a_disr, a_disr_max, a_max);
            break;

        default:
            if(dust_prob != 0)
                return findSizeID(pp, dust_prob[w], a_min, a_disr, a_disr_max, a_max);
            break;
    }
    ...
    uint a = findSizeID(pp, prob, a_min, a_disr, a_disr_max,  a_max);
                                               ...
                                               bool use_energy_density)
{
    ...
    uint a = getInteractingDust(grid, pp, i_density);
...
double CDustComponent::calcRATSpeed(CGridBasic * grid, cell_basic * cell, uint i_density, uint a, parameters & param)
{
    ...
    double tau_gas = 3. / (4 * PIsq) * I_p / (mu * n_g * m_H * v_th * Gamma_par * pow(a_eff[a], 4));
...
void CDustComponent::calcMaxAlignedRadii(CGridBasic * grid, cell_basic * cell, uint i_density)
{
	// Get local min and max grain sizes
    double a_min = getSizeMin(grid, cell);
    double a_max = getSizeMax(grid, cell);
...
void CDustComponent::calckRATlowJRadii(CGridBasic * grid, cell_basic * cell, uint i_density, parameters & param)
{
    ...
    if(getNumberDensity(grid, cell, i_density) == 0)
    {
        grid->setkRATlowJRadius(cell, i_density, a_max); 
        return;
    }
    
    // default value of the alignment radius
    double akrat_lowJ = getSizeMax(grid, cell);
    ...
    for(uint a = 0; a < nr_of_dust_species; a++)
    {
        if(sizeIndexUsed(a, a_min, a_max))
        {
            ...
            // Minor and major axis
            double a_minor = a_eff[a] * pow(s, 2. / 3.);    //[m]
            double a_major = a_eff[a] * pow(s, -1. / 3.);   //[m]
            ...
            double Gamma_RAT_e3 = CMathFunctions::integ(wavelength_list, arr_product, 0, nr_of_wavelength - 1);
            // Precession rate of J around radiation field
            double precession_rate = Gamma_RAT_e3 / J_th;
            // Precession timescale of J around radiation field
            double tau_rad = 2 * PI / precession_rate;
            ...
            double tau_Lar;
            double a_major_cgs = a_major * 1e2; // cm
            double rho_cgs = getMaterialDensity(a) * 1e-3; //g cm-3
            double Blen_cgs = Blen * 1e4; //G
            if (fp != 0)
            {
            ...
            double delta_t = tau_Lar/tau_rad;
            if(delta_t >=  1)
            {
                // linear interpolation
                if(a > 1)
                {
                    ...
                    double o1 = delta_t_old -  1;
                    double o2 = delta_t - 1;

                    akrat_lowJ = a1 - o1 * (a2 - a1) / (o2 - o1);
                }
                else
                    akrat_lowJ = a_min;
    ...
    // Set aligned grain size in grid
    grid->setkRATlowJRadius(cell, i_density, akrat_lowJ);
...
void CDustComponent::calckRAThighJRadii(CGridBasic * grid, cell_basic * cell, uint i_density,  parameters & param)
{
    ...
    if(getNumberDensity(grid, cell, i_density) == 0)
    {
        grid->setkRAThighJRadius(cell, i_density, a_max); 
        return;
    }
    
    // default value of the alignment radius
    double akrat_highJ = getSizeMax(grid, cell);
    ...
    // Check for proper size range
    if(akrat_highJ < a_min)
        akrat_highJ = a_min;

    if(akrat_highJ > a_max)
        akrat_highJ = a_max;

    // Set aligned grain size in grid
    grid->setkRAThighJRadius(cell, i_density, akrat_highJ);
                                               ...
                                               Vector3D dir_obs)
{
    ...
    {
        case PH_MIE:
            return getEscapePhotonMie(grid, pp, a, i_density, obs_ex, dir_obs);
        ...
        default:
        {
            ...
            while(grid->next(&pp_res))
            {
                ...
                dens = getNumberDensity(grid, &pp_res, i_density);

                // Get the current mean extinction cross-section
				Cext = getCextMean(grid, &pp_res, i_density);
