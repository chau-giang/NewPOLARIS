...
double CDustComponent::calcRATSpeed(CGridBasic * grid, cell_basic * cell, uint i_density, uint a)
{
    ...
    double tau_drag = tau_gas / (1. + FIR); //t_damp

	delete[] arr_product;
	delete[] du;
	
    // Saturated angular speed
 	return gamma_rat * tau_drag / I_p;   //w_RAT
}
...
double CDustComponent::calcDisruptRadii(CGridBasic * grid, cell_basic * cell, uint i_density, uint Smax)
{
    ...
    for(uint a = 0; a < nr_of_dust_species; a++)
    {
        ...
        {
            // Saturated angular speed
            double omega_rat = calcRATSpeed(grid, cell, i_density, a_eff[a]);
...
void CDustComponent::calcMaxDisruptRadii(CGridBasic * grid, cell_basic * cell, uint i_density)
{
    ...
    for(uint a = nr_of_dust_species; a > 0; a--)
    {
        if(sizeIndexUsed(a, a_min, a_max))
        {
            ...
            else if (omega_frac == DISRUPTION_LIMIT)
            {
                a_disr_max = a_eff[a];
                break;
            }
...
void CDustComponent::calcBarnetRadii(CGridBasic * grid, cell_basic * cell, uint i_density)
{
    // Calculate the aligned radii only for cells with a non-zero density
    //cout <<  getNumberDensity(grid, *cell, i_density) << endl;
    ...
    for(uint a = 0; a < nr_of_dust_species; a++)
    {
        ...
        {   
            ...
            {
            	T_dust = grid->getDustTemperature(*cell, i_density, a_eff[a]);
			}
			else
			{
            ...
            double omega_rat = pow((kb *T_dust / (I_p * (h-1))), 0.5);  // at low J attractor point with J = Jth
            
			// Volume of grain size a
			double V = 4 * PI / 3 * s * pow(a_eff[a], 3);
			...
			if (fp != 0) // grain is paramagnetic grains
 			{
				K_w = CMathFunctions::calc_K_w(T_dust, fp, omega_rat);	
			}
			else // grain is superparamagnetic grains
			{			
				K_w = CMathFunctions::calc_K_w_super(T_dust, Ncl, phi_sp, omega_rat);
			}
			
			// Barnet relaxation timescale [for	internal alignment]
			double t_bar = I_p * pow(gamma_g, 2) / (V * K_w * pow(h,2) * (h-1) * pow(omega_rat, 2));
